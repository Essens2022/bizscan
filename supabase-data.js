...
async function accessSummary(){const c=await getSupabaseClient();const u=await currentUser();if(!u)return{authenticated:false,credits:0,available_credits:0,available_pdf_credits:0,plan:'free',unlocked_analyses:0,subscription_active:false,unlocked:[]};const[{data:s,error:e1},{data:urows,error:e2}]=await Promise.all([c.rpc("get_my_access_summary"),c.from("analysis_unlocks").select("analysis_id,source,expires_at,analyses(slug,title,emoji)").eq("user_id",u.id)]);if(e1)throw e1;if(e2)throw e2;const summary=Array.isArray(s)?s[0]:(s||{});const availCredits=Number(summary.available_credits??summary.credits??0);return{authenticated:true,credits:availCredits,available_credits:availCredits,available_pdf_credits:Number(summary.available_pdf_credits??0),plan:summary.plan||'free',unlocked_analyses:Number(summary.unlocked_analyses??0),subscription_active:Boolean(summary.has_active_subscription??summary.subscription_active),has_active_subscription:Boolean(summary.has_active_subscription??summary.subscription_active),subscription_end:summary.subscription_end||null,unlocked:urows||[]}}
