...
function mapAnalysis(row,i=0){
  ...
  return {...row,attachments,coverUrl:cover?.file_url||"",category:cat.name||"",categorySlug:cat.slug||"",categoryEmoji:cat.emoji||"",risk:Number(row.risk_score)||0,score:Number(row.bizscan_score)||0,riskLabel:r.label,riskColor:r.color,visual:visualFor(row,i),investment:row.investment_display||"",profit:row.profit_display||row.profit_level||"",payback:row.payback_display||"",summary:row.short_description||"",price:Number(row.price??1.99),emoji:row.emoji||cat.emoji||"📊",verdictLabel:{conviene:"Conviene",dipende:"Dipende",non_conviene:"Non conviene"}[row.verdict]||"Analisi BizScan",displayData:row.display_data||null};
}
async function fetchDisplayDataMap(ids){
  if(!ids?.length)return{};
  const c=await getSupabaseClient();
  const{data,error}=await c.from("analysis_display_data").select("analysis_id,data").in("analysis_id",ids);
  if(error){console.warn("BizScan display data",error);return{}}
  return Object.fromEntries((data||[]).map(x=>[x.analysis_id,x.data||{}]));
}
async function fetchAnalysisDisplayData(analysisId){
  if(!analysisId)return{};
  const c=await getSupabaseClient();
  const{data,error}=await c.from("analysis_display_data").select("data").eq("analysis_id",analysisId).maybeSingle();
  if(error){console.warn("BizScan display data",error);return{}}
  return data?.data||{};
}
async function fetchPublishedAnalyses(){
  const c=await getSupabaseClient();const{data,error}=await c.rpc("public_list_analyses");if(error)throw error;
  const mapped=(data||[]).map(mapAnalysis),extra=await fetchDisplayDataMap(mapped.map(x=>x.id).filter(Boolean));
  return mapped.map(x=>({...x,displayData:extra[x.id]||x.displayData||{}}));
}
...
async function fetchAnalysisBySlug(slug){
  const c=await getSupabaseClient();const{data,error}=await c.rpc("public_get_analysis_by_slug",{p_slug:slug});if(error)throw error;if(!data)return null;
  let row=data;try{if(row.id&&(!Array.isArray(row.attachments)||!row.attachments.length))row={...row,attachments:await fetchAttachments(row.id)}}catch(e){console.warn("BizScan attachments fallback",e)}
  const mapped=mapAnalysis(row);mapped.displayData=await fetchAnalysisDisplayData(mapped.id);return mapped;
}
...
async function fetchPlans(){const c=await getSupabaseClient();const{data,error}=await c.from("plans").select("id,type,title,description,price,currency,analysis_limit,billing_interval,order_index").eq("is_active",true).order("order_index");if(error)throw error;return data||[]}
...
async function accessSummary(){const c=await getSupabaseClient();const u=await currentUser();if(!u)return{authenticated:false,credits:0,subscription_active:false,unlocked:[]};const[{data:s,error:e1},{data:urows,error:e2}]=await Promise.all([c.rpc("get_my_access_summary"),c.from("analysis_unlocks").select("analysis_id,source,expires_at,analyses(slug,title,emoji)").eq("user_id",u.id)]);if(e1)throw e1;if(e2)throw e2;const summary=Array.isArray(s)?s[0]:(s||{});return{authenticated:true,credits:Number(summary.available_credits??summary.credits??0),subscription_active:Boolean(summary.has_active_subscription??summary.subscription_active),subscription_end:summary.subscription_end||null,unlocked:urows||[]}}
...
window.BizScanData={getSupabaseClient,fetchPublishedAnalyses,fetchAnalysisBySlug,fetchAttachments,fetchCategories,fetchPlans,fetchAnalysisDisplayData,session,currentUser,fetchFavorites,setFavorite,accessSummary,hasAccess,unlockWithCredit,fetchSections,requestPdfAccess,getPdfAccessStatus,savePdfAccessRule,signedAttachmentUrl};
