...
async function applyCardStats(){
 try{
  ...
  document.querySelectorAll('[data-stats-field="rating"]').forEach(el=>{
   ...
   if(s&&s.review_count>0)el.innerHTML=`${s.avg_stars} ${renderStarsPartial(s.avg_stars)} (${s.review_count})`;
  });
...
async function loadAnalysisRatingAndPurchases(analysisId){
 ...
 try{
  ...
  if(ratingEl&&stats.review_count>0){
   ratingEl.innerHTML=`${stats.avg_stars} ${renderStarsPartial(stats.avg_stars)} (${stats.review_count})`;
  }
 }catch(e){console.warn('Errore caricamento rating',e)}
}
