...
window._doUnlockTool=async(toolKey)=>{
 ...
 try{
  ...
  if(!data?.success){
   ...
   if(data?.reason==='no_credits'){modal('Serve un credito per sbloccare','<p>Acquistando un pacchetto ricevi dei crediti e sblocchi subito gli indicatori inclusi nel tuo livello. Usa i crediti per sbloccare gli altri, uno alla volta.</p>','<a class="btn gold full" href="pricing.html">Vedi i pacchetti</a>');return}
...
window.unlockCompareAdvice=async(analysisIdA,analysisIdB)=>{
 confirmWithdrawalWaiver('',async()=>{
  ...
  if(!result.success){
   if(result.reason==='no_credits'){modal('Serve un credito per sbloccare','<p>Acquistando un pacchetto ricevi dei crediti e sblocchi subito gli indicatori inclusi nel tuo livello. Usa i crediti per sbloccare gli altri, uno alla volta.</p>','<a class="btn gold full" href="pricing.html">Vedi i pacchetti</a>');return}
