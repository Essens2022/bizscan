...
let PACKAGES=[
 ...
 {key:'smart',name:'Smart',price:6.99,analyses:18,pdfCredits:1,badge:'SCELTA INTELLIGENTE',indicatorCount:0,compare:'Base',features:['Tutti gli strumenti dei piani precedenti','18 crediti','Distribuzione costi iniziali','Confronto con la media categoria','1 credito report PDF']},
 {key:'pro',name:'Pro',price:9.99,analyses:26,pdfCredits:2,badge:'PIÙ SCELTO',indicatorCount:0,compare:'Base',features:['Tutti gli strumenti dei piani precedenti','26 crediti','Cash-flow a 12 mesi','Costi fissi e variabili mensili','2 crediti report PDF']},
 {key:'advanced',name:'Advanced',price:14.99,analyses:39,pdfCredits:3,badge:'CONSIGLIATO',indicatorCount:0,compare:'Base',features:['Tutti gli strumenti dei piani precedenti','39 crediti','Fabbisogno di personale','Fornitori e autorizzazioni','3 crediti report PDF']},
...
function image(p,wide=false,eager=false){const src=wide?(p.wideCover||p.coverUrl):p.coverUrl;return src?`<img src="${esc(src)}" alt="${esc(p.title)}" loading="${eager?'eager':'lazy'}" decoding="${eager?'sync':'async'}">`:`<div class="visual-placeholder">${p.categoryEmoji||p.emoji||'📊'}</div>`}
...
function preloadCardImages(){
 const promises=analyses.map(p=>{
  if(!p.coverUrl)return Promise.resolve();
  const img=new Image();
  img.src=p.coverUrl;
...
async function markSingleNotificationSeen(id,detailsEl){
 ...
 if(!n||n.seen_at)return; // già letta, nessuna chiamata inutile
 try{
  const c=await BizScanData.getSupabaseClient();
  await c.rpc('mark_notification_seen',{p_notification_id:id});
  n.seen_at=new Date().toISOString();
  // Rimuove solo la classe da QUESTO elemento specifico, senza ri-renderizzare l'intero
  // pannello (che richiuderebbe subito i <details> appena aperti, spezzando l'interazione).
...
let statsCache=null,statsCachePromise=null;
async function ensureStatsLoaded(){
 ...
 if(!statsCachePromise){
  statsCachePromise=(async()=>{
   try{
    const c=await BizScanData.getSupabaseClient();
    const{data}=await c.rpc('get_all_analyses_stats');
    const map={};
    (data||[]).forEach(s=>{map[s.analysis_id]=s});
    statsCache=map;
    return map;
   }catch(e){console.warn('Errore caricamento statistiche',e);statsCache={};return statsCache}
  })();
 }
 return statsCachePromise;
}
function card(p,idx=0){
 const url=`analysis.html?slug=${encodeURIComponent(p.slug)}`;
 const s=statsCache&&statsCache[p.id];
 const ratingHtml=(s&&s.review_count>0)?`${s.avg_stars} ${renderStarsPartial(s.avg_stars)} (${s.review_count})`:'';
 const purchasesHtml=s?`${s.purchase_count} acquisti`:'';
 return `<article class="business-card"><div class="business-cover business-cover-link" role="link" tabindex="0" aria-label="Apri ${esc(p.title)}" onclick="location.href='${url}'" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();location.href='${url}'}">${image(p,false,idx<3)}<div class="card-purchases-badge">${purchasesHtml}</div><div class="card-tools"><button aria-label="Confronta" onclick="event.preventDefault();event.stopPropagation();toggleCompare('${p.slug}')" data-compare-slug="${p.slug}">${compare.includes(p.slug)?'✓':'⇄'}</button><button aria-label="Salva" data-fav-slug="${p.slug}" onclick="event.preventDefault();event.stopPropagation();toggleFavorite('${p.slug}')">${favorites.includes(p.slug)?'♥':'♡'}</button></div></div><a href="${url}" class="business-body"><div class="business-title-row"><div><h3>${esc(p.title)}</h3><div class="category-row"><span class="category">${esc(p.category||'Business')}</span><span class="card-rating">${ratingHtml}</span></div></div><span class="card-open-arrow">→</span></div><div class="card-metrics">${scoreRing(p.score,'small')}<div class="metric-list"><span><em>Investimento</em><b>${esc(p.investment||'—')}</b></span><span><em>Profitto annuo</em><b>${esc(p.profit||'—')}</b></span><span><em>Rischio</em><b class="${riskClass(p)}">${esc((p.riskLabel||'—').replace('Rischio ',''))}</b></span></div></div></a></article>`
}
...
window.setHomeFilter=async filter=>{
 homeFilter=filter;
...
function sizeToEm(size){
 ...
 if(typeof size==='string'&&SEGMENT_SIZE_EM[size]!=null)return SEGMENT_SIZE_EM[size];
...
function initHeroRotation(){
 ...
 const carousel=$('.hero-media-carousel');
 const startMediaTimer=()=>{
  clearInterval(_heroMediaTimer);
  if(heroMedia.length<=1)return;
  _heroMediaTimer=setInterval(()=>goToSlide(_heroMediaIdx+1),Math.max(3,Math.min(10,heroSeconds))*1000);
 };
 function goToSlide(idx){
  ...
  _heroMediaIdx=((idx%slides.length)+slides.length)%slides.length;
  slides[_heroMediaIdx]?.classList.add('active');dots[_heroMediaIdx]?.classList.add('active');
 }
 if(heroMedia.length>1){
  startMediaTimer();
  if(carousel){
   let touchStartX=0,touchDeltaX=0,resumeTimer=null;
   const pause=()=>{clearInterval(_heroMediaTimer);clearTimeout(resumeTimer)};
   const scheduleResume=()=>{clearTimeout(resumeTimer);resumeTimer=setTimeout(startMediaTimer,2500)};
   carousel.addEventListener('touchstart',e=>{touchStartX=e.touches[0].clientX;touchDeltaX=0;pause()},{passive:true});
   carousel.addEventListener('touchmove',e=>{touchDeltaX=e.touches[0].clientX-touchStartX},{passive:true});
   carousel.addEventListener('touchend',()=>{
    if(Math.abs(touchDeltaX)>40){touchDeltaX<0?goToSlide(_heroMediaIdx+1):goToSlide(_heroMediaIdx-1)}
    scheduleResume();
   },{passive:true});
   let mouseDownX=null;
   carousel.addEventListener('mousedown',e=>{mouseDownX=e.clientX;pause()});
   carousel.addEventListener('mouseup',e=>{
    if(mouseDownX!==null){
     const dx=e.clientX-mouseDownX;
     if(Math.abs(dx)>40){dx<0?goToSlide(_heroMediaIdx+1):goToSlide(_heroMediaIdx-1)}
    }
    mouseDownX=null;scheduleResume();
   });
   carousel.addEventListener('mouseleave',()=>{if(mouseDownX!==null){mouseDownX=null;scheduleResume()}});
   $$('.hero-media-dots span').forEach((dot,i)=>dot.addEventListener('click',()=>{goToSlide(i);pause();scheduleResume()}));
  }
 }
 ...
 host.innerHTML=`<div class="home18">
  ...
  <section class="home18-section">
   ...
   <div class="home18-dashboard">
    ...
    <article class="home18-chart-card"><small>SCENARI DI PROFITTO</small><div class="home18-bars-chart"><i style="height:34%"></i><i style="height:48%"></i><i style="height:57%"></i><i style="height:72%"></i><i style="height:88%"></i><i style="height:78%"></i></div><div class="home18-chart-labels"><span>Piccola</span><span>Media</span><span>Grande</span></div></article>
   </div>
  </section>
  ...
  <section class="home18-section home18-testimonials">
   ...
   <div class="site-fb-actions">
    <div class="feedback-form-card" id="siteFbFormCard"><h3>Lascia il tuo feedback su BizScan</h3><div class="feedback-form-row feedback-stars-row"><small>La tua valutazione</small><div class="feedback-star-picker" id="siteFbStarPicker" data-value="0">${starPickerHtml()}</div></div><div class="feedback-form-row"><input type="text" id="siteFbName" placeholder="Nome e cognome" maxlength="60"></div><div class="feedback-form-row"><textarea id="siteFbMessage" placeholder="La tua esperienza con BizScan…" maxlength="500" rows="3" oninput="updateCharCounter('siteFbMessage')"></textarea><div class="char-counter-row">${charCounter('siteFbMessage',500)}${emojiRow('siteFbMessage')}</div></div><button class="btn gold full" onclick="submitSiteFeedback()">Invia feedback</button><p class="feedback-form-msg" id="siteFbFormMsg"></p></div>
    <div class="suggestion-section"><h3>Aiutaci a migliorare BizScan</h3><p>Hai un'idea, hai trovato un problema, o vuoi suggerirci qualcosa?</p><textarea id="homeSuggestionMessage" placeholder="Cosa possiamo migliorare o aggiungere?" maxlength="1000" rows="3" oninput="updateCharCounter('homeSuggestionMessage')"></textarea><div class="char-counter-row">${charCounter('homeSuggestionMessage',1000)}${emojiRow('homeSuggestionMessage')}</div><button class="btn ghost" onclick="submitHomeSuggestion()">Invia suggerimento</button><p class="feedback-form-msg" id="homeSuggestionMsg"></p></div>
   </div>
  </section>
 </div>`
 initHeroRotation()
...
window.toggleEmojiPanel=function(ev,textareaId){
 ev.stopPropagation();
 const panel=document.getElementById('emojiPanel-'+textareaId);if(!panel)return;
...
function starPickerHtml(n){
 let s='';
 for(let i=1;i<=5;i++){
  s+=`<span data-star="${i}"><svg viewBox="0 0 24 24" width="26" height="26"><path d="M12 2.5l2.9 6 6.6.7-4.9 4.5 1.3 6.5-5.9-3.3-5.9 3.3 1.3-6.5-4.9-4.5 6.6-.7z" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/></svg></span>`;
 }
 return s;
}
...
function renderStarsPartial(n){
 const rounded=Math.floor(n*2)/2; // arrotonda per DIFETTO alla mezza stella più vicina - non deve mai mostrare più di quanto sia la media reale
 ...
 try{
  ...
  if(avgBox){
   if(s&&Number(s.total_count)>0){
    avgBox.innerHTML=`<span class="feedback-avg-num">${s.avg_stars}</span><span class="feedback-avg-stars">${renderStarsPartial(s.avg_stars)}</span><small>${s.total_count} recensioni</small>`;
   }else{
 ...
 try{
  ...
  if(avgBox){
   avgBox.innerHTML=(s&&Number(s.total_count)>0)
    ?`<span class="feedback-avg-num">${s.avg_stars}</span><span class="feedback-avg-stars">${renderStarsPartial(s.avg_stars)}</span><small>${s.total_count} recensioni</small>`
    :`<span class="feedback-avg-num">—</span><span class="feedback-avg-stars">☆☆☆☆☆</span><small>nessuna recensione</small>`;
  }
  const {data:top}=await supabaseClient.rpc('public_top_site_feedback',{p_limit:1000});
...
function analysisOverview(p){
 ...
 const investPerScaleHtml=`<div class="invest-per-scale"><h4>Investimento per scala</h4><div class="invest-per-scale-row">${investCol('Piccola','prudente')}${investCol('Media','realistico')}${investCol('Grande','ottimistico')}</div></div>`;
...
window._doUnlockTool=async(toolKey)=>{
 ...
 try{
  ...
  if(!data?.success){
   if(loadingWrap)loadingWrap.style.opacity='1';
   if(data?.reason==='no_credits'){modal('Serve un credito per sbloccare','<p>Acquistando un pacchetto sblocchi gli indicatori inclusi nel tuo livello e ricevi dei crediti, con cui puoi sbloccare gli altri uno alla volta.</p>','<a class="btn gold full" href="pricing.html">Vedi i pacchetti</a>');return}
...
function toolPreview(type){
 const previews={
  ...
  payback:'<div class="tool-ring cyan" style="--v:56"><b>2,8</b><small>anni</small></div>',
  risk:'<div class="tool-risk"><i><em style="left:56%"></em></i><b>Rischio medio</b><small>2,8 / 5</small></div>',
  ...
  stress:'<div class="tool-gauge"><i class="gauge-needle" style="transform:rotate(-9deg)"></i><b>Stress test</b></div>',
  market:'<div class="tool-market-vis"><div><small>Domanda</small><div class="mini-ring" style="--v:78"><b>Alta</b></div></div><div><small>Concorrenza</small><div class="mini-ring amber" style="--v:50"><b>Media</b></div></div></div>',
  operations:'<div class="tool-market-vis"><div><small>Personale</small><div class="mini-ring amber" style="--v:50"><b>Medio</b></div></div><div><small>Complessità</small><div class="mini-ring" style="--v:60"><b>3/5</b></div></div></div>',
...
window.unlockCompareAdvice=async(analysisIdA,analysisIdB)=>{
 confirmWithdrawalWaiver('',async()=>{
  ...
  if(!result.success){
   if(result.reason==='no_credits'){modal('Serve un credito per sbloccare','<p>Acquistando un pacchetto sblocchi gli indicatori inclusi nel tuo livello e ricevi dei crediti, con cui puoi sbloccare gli altri uno alla volta.</p>','<a class="btn gold full" href="pricing.html">Vedi i pacchetti</a>');return}
