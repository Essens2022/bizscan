...
function renderHome(){
 ...
 host.innerHTML=`<div class="home18">
  <section class="home18-hero">
   ...
   }).join('')}${heroMedia.length>1?`<div class="hero-media-dots">${heroMedia.map((_,i)=>`<span class="${i===0?'active':''}" data-dot="${i}"></span>`).join('')}</div>`:''}</div><div class="hero-wave"><svg viewBox="0 0 1200 60" preserveAspectRatio="none"><defs><linearGradient id="waveGrad" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stop-color="#ffb400"/><stop offset="100%" stop-color="#ff8a00"/></linearGradient></defs><path d="M0,30 C300,0 900,60 1200,30 L1200,60 L0,60 Z" fill="url(#waveGrad)"></path></svg></div>`:''}
