...
window._doUnlockTool=async(toolKey)=>{
 ...
 try{
  ...
  if(!data?.success){
   ...
   if(data?.reason==='no_credits'){modal('Serve un credito per sbloccare','<p>I crediti si ottengono acquistando uno dei pacchetti disponibili. Ogni pacchetto include già l&#39;accesso automatico agli indicatori del suo livello: i pacchetti più completi sbloccano indicatori più avanzati senza consumare crediti aggiuntivi. I crediti restanti puoi usarli per sbloccare singolarmente indicatori o report PDF di altre analisi.</p>','<a class="btn gold full" href="pricing.html">Vedi i pacchetti</a>');return}
...
window.unlockCompareAdvice=async(analysisIdA,analysisIdB)=>{
 confirmWithdrawalWaiver('',async()=>{
  ...
  if(!result.success){
   if(result.reason==='no_credits'){modal('Serve un credito per sbloccare','<p>I crediti si ottengono acquistando uno dei pacchetti disponibili. Ogni pacchetto include già l&#39;accesso automatico agli indicatori del suo livello: i pacchetti più completi sbloccano indicatori più avanzati senza consumare crediti aggiuntivi. I crediti restanti puoi usarli per sbloccare singolarmente indicatori o report PDF di altre analisi.</p>','<a class="btn gold full" href="pricing.html">Vedi i pacchetti</a>');return}
