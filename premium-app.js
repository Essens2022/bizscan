...
window.toggleFavorite=async slug=>{const p=analyses.find(x=>x.slug===slug);const enabling=!favorites.includes(slug);favorites=enabling?[...favorites,slug]:favorites.filter(x=>x!==slug);localStorage.setItem('bizscan_favorites',JSON.stringify(favorites));if(access.authenticated&&p?.id){try{await BizScanData.setFavorite(p.id,enabling)}catch(e){console.warn(e)}}toast(enabling?'Aggiunta ai preferiti':'Rimossa dai preferiti');
 document.querySelectorAll(`[data-fav-slug="${slug}"]`).forEach(btn=>{
  if(btn.classList.contains('outline-gold')){btn.innerHTML=enabling?'♥ Salvato':'♡ Salva'}
   ...
   <div class="home18-dashboard">
    <article class="home18-score-card">
     ...
     <div class="home18-bars">
      ...
      <div><span>Profitto</span><i><b class="profit" style="width:${mb.profit}%"></b></i><strong>${mb.profit}</strong></div>
      <div><span>ROI</span><i><b class="roi" style="width:${mb.roi}%"></b></i><strong>${mb.roi}</strong></div>
      <div><span>Rientro</span><i><b class="payback" style="width:${mb.payback}%"></b></i><strong>${mb.payback}</strong></div>
     </div>
    </article>
