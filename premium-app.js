...
async function loadAnalysisRatingAndPurchases(analysisId){
 if(!analysisId)return;
 try{
  const supabaseClient=await BizScanData.getSupabaseClient();
  const {data}=await supabaseClient.rpc('get_analysis_stats',{p_analysis_id:analysisId});
  const stats=data&&data[0];
  if(!stats)return;
  const ratingEl=document.getElementById('analysisRatingMini');
  if(ratingEl&&stats.review_count>0){
   ratingEl.innerHTML=`${stats.avg_stars} ${renderStars(stats.avg_stars)} (${stats.review_count})`;
  }
  const purchasesEl=document.getElementById('analysisPurchasesMini');
  if(purchasesEl){
   purchasesEl.textContent=`${stats.purchase_count} ACQUISTI`;
  }
 }catch(e){console.warn('Errore caricamento rating/acquisti',e)}
}
