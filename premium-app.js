 ...
 host.innerHTML=`<div class="home18">
  ...
  <section class="home18-section home18-testimonials">
   ...
   <div class="site-fb-actions">
    <div class="feedback-form-card" id="siteFbFormCard"><h3>Lascia il tuo feedback su BizScan</h3><div class="feedback-form-row feedback-stars-row"><small>La tua valutazione</small><div class="feedback-star-picker" id="siteFbStarPicker" data-value="0"><span data-star="1">★</span><span data-star="2">★</span><span data-star="3">★</span><span data-star="4">★</span><span data-star="5">★</span></div></div><div class="feedback-form-row"><input type="text" id="siteFbName" placeholder="Nome e cognome" maxlength="60"></div><div class="feedback-form-row"><textarea id="siteFbMessage" placeholder="La tua esperienza con BizScan…" maxlength="500" rows="3"></textarea>${emojiRow('siteFbMessage')}</div><button class="btn gold full" onclick="submitSiteFeedback()">Invia feedback</button><p class="feedback-form-msg" id="siteFbFormMsg"></p></div>
    <div class="suggestion-section"><h3>Aiutaci a migliorare BizScan</h3><p>Hai un'idea, hai trovato un problema, o vuoi suggerirci qualcosa?</p><textarea id="homeSuggestionMessage" placeholder="Cosa possiamo migliorare o aggiungere?" maxlength="1000" rows="3"></textarea>${emojiRow('homeSuggestionMessage')}<button class="btn ghost" onclick="submitHomeSuggestion()">Invia suggerimento</button><p class="feedback-form-msg" id="homeSuggestionMsg"></p></div>
   </div>
  </section>
 </div>`
 initHeroRotation()
...
async function initFeedbackSection(slug){
 ...
 try{
  ...
  if(carousel){
   if(top&&top.length){
    carousel.innerHTML=top.map(f=>`<div class="feedback-card"><div class="feedback-card-stars">${renderStars(f.stars)}</div><p class="feedback-card-msg">${esc(f.message)}</p><div class="feedback-card-foot"><b>${esc(f.display_name)}${f.is_official?' <svg class="verified-badge" viewBox="0 0 24 24" width="13" height="13" fill="#1d9bf0" aria-label="Verificato"><path d="M12 1.5l2.6 1.3 2.9-.4 1.3 2.6 2.6 1.3-.4 2.9 1.3 2.6-1.9 2.2.4 2.9-2.9.4-1.9 2.2-2.6-1.3-2.9.4-1.3-2.6-2.6-1.3.4-2.9-1.3-2.6 1.9-2.2-.4-2.9 2.9-.4z"/><path d="M9 12l2 2 4-4" stroke="#fff" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg>':''}</b><small>${timeAgo(f.created_at)}</small></div>${f.admin_reply?`<div class="feedback-reply"><b>Amministrazione BizScan <svg class="verified-badge" viewBox="0 0 24 24" width="12" height="12" fill="#1d9bf0" aria-label="Verificato"><path d="M12 1.5l2.6 1.3 2.9-.4 1.3 2.6 2.6 1.3-.4 2.9 1.3 2.6-1.9 2.2.4 2.9-2.9.4-1.9 2.2-2.6-1.3-2.9.4-1.3-2.6-2.6-1.3.4-2.9-1.3-2.6 1.9-2.2-.4-2.9 2.9-.4z"/><path d="M9 12l2 2 4-4" stroke="#fff" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round"/></svg></b><p>${esc(f.admin_reply)}</p></div>`:''}</div>`).join('');
   }
  }
